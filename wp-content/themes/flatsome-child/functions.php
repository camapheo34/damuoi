...
function devvn_viewnumber_func($atts){
    ...
    if ($cate_id) {
        ...
        $category = get_term($cate_id);
        $html_list = '<h3 class="title">'.$category->name.'</h3>';
    }
    $storesList = array();
    $stores = get_posts($args);
    $cl_dvls = new DevVN_Local_Store_Class();
    if($stores && !is_wp_error($stores)){
        $html_list .= '<ul>';
        foreach ($stores as $store){
            $storeItem = $cl_dvls->dvls_get_store_infor($store->ID);
            $url_show_map = 'https://www.google.com/maps/search/?api=1&query='.$storeItem['address'];
            $html_list .= '<li>';
            $html_list .= '<span class="name">'.$storeItem['name'].'</span><br />';
            $html_list .= '<span class="address">'.$storeItem['address'].'</span><br />';
            $html_list .= '<a href="'.$url_show_map.'" target="_blank" class="direction"><i class="fa fa-map-marker" aria-hidden="true"></i> Xem bản đồ</a>';
            $html_list .= '</li>';
        }
        $html_list .= '</ul>';
    }
